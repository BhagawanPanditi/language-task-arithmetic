...
class FlatArguments:
    ...
    dataset_mixer_list: List[str] = field(default_factory=lambda: ["/workspace/ibm-vaibhav/xnli_messages_en_train.jsonl", "1.0"])
...
############################################################################################
    ...
    # --- Validation-specific args ---
    # val_dataset_mixer_list: Optional[List[str]] = field(
    #     default_factory=lambda: ["/workspace/ibm-vaibhav/formatted_datasets/tulu-math-persona/final_validation.jsonl", "1.0","/workspace/ibm-vaibhav/formatted_datasets/xlsum/final_validation.jsonl","1.0", "/workspace/ibm-vaibhav/formatted_datasets/xnli/final_validation.jsonl","1.0"],
    #     metadata={
    #         "help": (
    #             "A list of validation dataset identifiers (HF id or local path). "
    #             "Each entry will be loaded separately as its own validation dataset. "
    #             "Example: ['data/val1.json', 'data/val2.json', 'allenai/some-val-ds']"
    #         )
    #     },
    # )
    val_dataset_mixer_list: Optional[List[str]] = field(
        default_factory=lambda: ["/workspace/ibm-vaibhav/xnli_messages_en_validation.jsonl", "1.0"],
        metadata={
            "help": (
                "A list of validation dataset identifiers (HF id or local path). "
                "Each entry will be loaded separately as its own validation dataset. "
    ...
    val_dataset_mixer_list_splits: List[str] = field(
        default_factory=lambda: ["train"],
        metadata={"help": "Which splits to load for val datasets (when applicable)."},
    )
    ...
    eval_steps: Optional[int] = field(
        default=None,
        metadata={
            "help": "Run validation every N completed steps. If None, uses logging_steps (if set)."
        },
    )
    max_val_samples: Optional[int] = field(
        default=None,
        metadata={"help": "If set, truncate each validation dataset to this many examples for faster evaluation."},
    )
############################################################################################
...
def run_validation(val_dataloader, model, accelerator, step, args):
    """
    Run validation on a single dataloader. Returns a dict of metrics.
    """
    model.eval()
    all_metrics = {}

    # accumulate totals on local process
    local_sum_loss = torch.tensor(0.0, device=accelerator.device)
    local_fwd = torch.tensor(0, dtype=torch.long, device=accelerator.device)

    for batch in val_dataloader:
        # make sure labels and batch tensors are already on device by prepared dataloader
        with torch.no_grad():
            outputs = model(**batch, use_cache=True)
            loss = outputs.loss

        local_sum_loss += loss.detach().float()
        local_fwd += 1

    # gather scalars across processes
    global_sum_loss = accelerator.gather(local_sum_loss).sum().item()
    global_fwd = accelerator.gather(local_fwd).sum().item()

    avg_loss = float(global_sum_loss / global_fwd) if global_fwd > 0 else float("nan")
    ppl = float(math.exp(avg_loss)) if avg_loss < 100 else float("inf")

    all_metrics["val_loss"] = avg_loss
    all_metrics["val_ppl"] = ppl

    # print from main process
    if accelerator.is_local_main_process:
        logger.info(f"[Validation] step={step} avg_loss={avg_loss:.6f} ppl={ppl:.3f}")

    # log metrics to wandb or other trackers
    if args.with_tracking:
        accelerator.log(all_metrics, step=step)
...
def main(args: FlatArguments, tc: TokenizerConfig):
    ...
    print("Tokenizer name or path:", tc.tokenizer_name_or_path)
...
############################################################################################
        val_dataset = get_cached_dataset_tulu(
            dataset_mixer_list=args.val_dataset_mixer_list,
            dataset_mixer_list_splits=args.val_dataset_mixer_list_splits,
            tc=tc,
            dataset_transform_fn=args.dataset_transform_fn,
            transform_fn_args=transform_fn_args,
            target_columns=args.dataset_target_columns,
...
############################################################################################
    ...
    layerwise_masks_up = torch.load("/workspace/ibm-vaibhav/language_mask.pt")
    layerwise_masks_gate = torch.load("/workspace/ibm-vaibhav/language_mask.pt")
    ...
    for layer_idx, layer in enumerate(model.model.layers):
        ...
        layer.mlp.gate_proj.weight.register_hook(
            lambda g, m=layerwise_masks_gate[layer_idx]: g * m.to(g.device, dtype=g.dtype)
        )
    ...
    val_dataloader = DataLoader(
        val_dataset, shuffle=True, collate_fn=DataCollatorForSeq2Seq(tokenizer=tokenizer, model=model, padding="longest"), batch_size=args.per_device_train_batch_size,drop_last=True
    )
    # Optimizer
    ...
    # )
############################################################################################
    to_prepare = [model, optimizer, train_dataloader, lr_scheduler, val_dataloader]
    prepared = accelerator.prepare(*to_prepare)
    model, optimizer, train_dataloader, lr_scheduler, val_dataloader = prepared
############################################################################################
        ...
        for step, batch in enumerate(active_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                if isinstance(checkpointing_steps, int):
                    ...
                    effective_eval_steps = int(args.eval_steps)
                    if completed_steps % effective_eval_steps == 0:
                        val_metrics = run_validation(val_dataloader, model, accelerator, completed_steps, args)
